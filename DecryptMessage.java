...
class Solution {
  ...
  static String decrypt(String word) {
    ...
    if (word.length()==0)
     ...
     int prevasciivalue=(int)prev;

    StringBuilder builder=new StringBuilder();


    builder.append((char)(prevasciivalue-1));



    for (int i=1;i<word.length();i++){
      int currascivalue=(int)word.charAt(i);

      while (currascivalue-prevasciivalue<97){
        currascivalue+=26;
      }

      char currchar=(char)(currascivalue-prevasciivalue);

      builder.append(currchar);
      prevasciivalue=currascivalue;
